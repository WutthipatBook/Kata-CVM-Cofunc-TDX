...
PATCH_FILE=${PATCH_FILE:-$BUNDLE/patches/host-kernel/0014-Diagnostic-clear-tdx-unpin-page-flags.patch}
JOBS=${JOBS:-16}
STAMP=$(date -u +%Y%m%d_%H%M%S)
BACKUP_DIR=${BACKUP_DIR:-$BUNDLE/backups/host-kernel-clear-tdx-unpin-flags-$STAMP}
MARKER='CoFunc old-ABI TDX unpin page-flag cleanup diagnostic'
...
--apply-build remounts /mnt/new_disk read-write if needed, applies the TDX
unpin page-flag cleanup diagnostic to the 5.19 source tree, and rebuilds
arch/x86/kvm.
...
as_tree_owner() {
	local owner
	owner=$(stat -c %U "$SRC/arch/x86/kvm/vmx/tdx.c")
...
patch_state() {
	if rg -q "$MARKER" "$SRC/arch/x86/kvm/vmx/tdx.c"; then
		echo "applied"
	else
		echo "not-applied"
	fi
...
check_state() {
	...
	echo "0014 TDX unpin flag-cleanup state: $(patch_state)"
	echo "0006 private-memfile flag-cleanup state: $(conflict_0006_state)"
...
apply_patch_file() {
	...
	sha256sum "$SRC/arch/x86/kvm/vmx/tdx.c" >"$BACKUP_DIR/source.before.sha256"
	sha256sum "$PATCH_FILE" >"$BACKUP_DIR/patch.sha256"

	if [[ $(patch_state) == applied ]]; then
		die "0014 diagnostic already appears applied"
	fi
	if [[ $(conflict_0006_state) == applied ]]; then
		die "0006 private-memfile flag cleanup appears applied; reverse it before applying this diagnostic"
	fi

	log "applying TDX unpin page-flag cleanup diagnostic"
	as_tree_owner patch -d "$SRC" -p1 -i "$PATCH_FILE"
	rg -n "$MARKER|cofunc_tdx_clear_unpin_page_flags|ClearPageReferenced|ClearPageDirty" \
		"$SRC/arch/x86/kvm/vmx/tdx.c" >"$BACKUP_DIR/source.after.rg"
	sha256sum "$SRC/arch/x86/kvm/vmx/tdx.c" >"$BACKUP_DIR/source.after.sha256"
...
reverse_patch_file() {
	...
	sha256sum "$SRC/arch/x86/kvm/vmx/tdx.c" >"$BACKUP_DIR/source.before-reverse.sha256"

	if [[ $(patch_state) != applied ]]; then
		die "0014 diagnostic does not appear applied"
	fi

	log "reversing TDX unpin page-flag cleanup diagnostic"
	as_tree_owner patch -R -d "$SRC" -p1 -i "$PATCH_FILE"
	sha256sum "$SRC/arch/x86/kvm/vmx/tdx.c" >"$BACKUP_DIR/source.after-reverse.sha256"
