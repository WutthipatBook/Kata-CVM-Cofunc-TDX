#!/usr/bin/env bash
# One-shot Fig. 11 face-detection measurement under the loaded patch-0023/0025
# modules: one excluded cold warm-up and exactly twenty measured cold launches.
# Every launch has an independent dmesg delta and cleanup check; no retry occurs.
set -Eeuo pipefail
...
WORKLOAD=fn_py_face_detection
EXPECTED_SAMPLES=20
run_root=${RUN_ROOT:-/home/booklyn/BookArchive/StageBreakdownRuns/kata_tdx_0025_face_measurement_$(date -u +%Y%m%d_%H%M%S)}
batch_token=${RUN_TOKEN:-f25-$(date -u +%H%M%S)-$((BASHPID % 100))}
...
make_delta() {
	...
	[[ -r $run_root/dmesg-$before.log && -r $run_root/dmesg-$after.log ]] ||
...
reject_delta() {
	local re=$1 description=$2 delta=$3 output=$4
	if rg -n -i "$re" "$delta" >"$output"; then
		fail "$description"
	fi
}

check_delta() {
	local delta=$1 label=$2 safe_label=${2//\//_}
	[[ -r $delta ]] || fail "missing kernel delta for $label: $delta"
	reject_delta "$stop_re" "canonical KVM/TDX stop marker after $label" "$delta" \
		"$run_root/${safe_label}-stop-markers.txt"
	reject_delta "$log_loss_re" "kernel log loss after $label" "$delta" \
		"$run_root/${safe_label}-log-loss.txt"
	reject_delta "$level2_re" "private level-2 mapping evidence after $label" "$delta" \
		"$run_root/${safe_label}-private-level2.txt"
	reject_delta "$promotion_re" "private 2M promotion evidence after $label" "$delta" \
		"$run_root/${safe_label}-private-2m-promotion.txt"
}

verify_phase_no_leftovers() {
	local label=$1 safe_label=${1//\//_} file
...
verify_sample_set() {
	...
	[[ -r "$root/kata_launch.log" ]] || { printf 'error: missing measured analyzer log\n' >&2; return 1; }
	...
	' "$root/kata_launch.log" >/dev/null || { printf 'error: invalid measured analyzer JSON\n' >&2; return 1; }
	for ((iteration = 1; iteration <= EXPECTED_SAMPLES; iteration++)); do
		...
		for marker in t_launch_begin t_import_begin t_func_load_begin t_import_done t_func_done; do
			rg -q "^${marker} [0-9]" "$sample_log" || {
				printf 'error: missing %s in %s\n' "$marker" "$sample_log" >&2; return 1;
			}
		done
	done
	[[ -r "$warmup_root/kata_launch.log" ]] || { printf 'error: missing untimed warm-up analyzer log\n' >&2; return 1; }
	[[ $(jq -s 'length' "$warmup_root/kata_launch.log") == 1 ]] || {
		printf 'error: warm-up analyzer count is not one\n' >&2; return 1;
	}
	[[ -r "$warmup_root/sample-001/container-exit-code.txt" && $(<"$warmup_root/sample-001/container-exit-code.txt") == 0 ]] || {
...
run_prebuild() {
	...
	set +e
	RUN_DIR="$run_root/prebuild/attempt-001" LOG_ROOT="$run_root/prebuild-log" \
	RUN_TOKEN="${batch_token}-prebuild" PREPARE_ONLY=1 SKIP_PREPARE=0 \
	RUNTIME_ENDPOINT="$RUNTIME_ENDPOINT" "$GENERIC_RUNNER" "$WORKLOAD" 1
	run_rc=$?
	set -e
...
run_launch() {
	...
	set +e
	RUN_DIR="$child_dir" LOG_ROOT="$log_root" RUN_TOKEN="${batch_token}-${label}" \
	PREPARE_ONLY=0 SKIP_PREPARE=1 START_ITERATION="$start_iteration" \
	RUNTIME_ENDPOINT="$RUNTIME_ENDPOINT" "$GENERIC_RUNNER" "$WORKLOAD" 1
	phase_rc=$?
	set -e
	...
	if (( phase_rc != 0 )); then run_rc=$phase_rc; exit "$phase_rc"; fi
}
...
finish() {
	...
	if [[ -r $run_root/dmesg-before.log ]]; then
		...
		if [[ -r $run_root/dmesg-after.log ]]; then
			...
			: >"$run_root/dmesg-after.delta"; evidence_rc=1
		fi
	else
		: >"$run_root/dmesg-after.delta"; evidence_rc=1
	fi
	capture_cri after
	sudo -n "$GATE" post-0025-face-measurement 2>&1 | tee "$run_root/postflight.log"
	...
	verify_final_no_leftovers
	if (( run_rc == 0 )); then verify_sample_set || evidence_rc=1; fi
	{
		printf 'run_rc=%d\n' "$run_rc"
...
for command in curl crictl diff dmesg find jq ps rg sed sha256sum tee wc; do need "$command"; done
...
} >"$run_root/face-measurement-command.txt"
cp "$0" "$run_root/harness.sh"
sha256sum "$0" >"$run_root/harness.sha256"

sudo -n "$GATE" pre-0025-face-measurement-launch | tee "$run_root/preflight.log"
{
	...
	curl --connect-timeout 5 --max-time 15 -fsS http://127.0.0.1:9000/minio/health/ready >/dev/null
...
# Preparation stages/imports the image without creating a CRI pod or timing it.
run_prebuild
