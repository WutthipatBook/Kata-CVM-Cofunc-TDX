...
PATCH_FILE=${PATCH_FILE:-$BUNDLE/patches/host-kernel/0009-Diagnostic-narrow-private-tdp-logging-to-face-gfns.patch}
JOBS=${JOBS:-16}
STAMP=$(date -u +%Y%m%d_%H%M%S)
BACKUP_DIR=${BACKUP_DIR:-$BUNDLE/backups/host-kernel-log-face-gfn-private-tdp-levels-$STAMP}
MARKER='CoFunc old-ABI face-GFN private TDP level diagnostic'
MMU_MARKER='CoFunc old-ABI face-GFN private MMU level diagnostic'
BASE_0008_MARKER='CoFunc old-ABI private TDP level diagnostic'
BASE_0008_MMU_MARKER='CoFunc old-ABI private MMU level diagnostic'
...
--apply-build remounts /mnt/new_disk read-write if needed, applies the 0009
face-GFN range overlay on top of the already-applied 0008 high-GFN private
TDP/MMU level diagnostic, and rebuilds arch/x86/kvm.

--reverse-build reverses only the 0009 overlay, returning the source to the
0008 diagnostic state. It does not install or reload modules; use
install_5_19_patched_kvm_modules.sh for that.
USAGE
}
...
base_0008_state() {
	if rg -q "$BASE_0008_MARKER|$BASE_0008_MMU_MARKER" "$SRC/arch/x86/kvm/mmu/mmu.c" "$SRC/arch/x86/kvm/mmu/tdp_mmu.c"; then
		echo "applied"
	else
		echo "not-applied"
	fi
...
check_state() {
	...
	echo "0009 face-GFN overlay state: $(patch_state)"
	echo "0008 high-GFN base state: $(base_0008_state)"
...
apply_patch_file() {
	...
	if [[ $(patch_state) == applied ]]; then
		die "0009 overlay already appears applied"
	fi
	if [[ $(base_0008_state) != applied ]]; then
		die "0008 high-GFN diagnostic must be applied before this overlay"
	fi
	if [[ $(conflict_0007_state) == applied ]]; then
		die "0007 private-fault-level diagnostic appears applied; reverse it before applying this overlay"
	fi
	if [[ $(conflict_0006_state) == applied ]]; then
		die "0006 flag-cleanup diagnostic appears applied; reverse it before applying this overlay"
	fi
	if [[ $(legacy_ad_state) == applied ]]; then
		die "legacy 0005 skip-dirty/accessed diagnostic appears applied; reverse it before applying this overlay"
	fi
	if [[ $(legacy_accessed_state) == applied ]]; then
		die "legacy 0004 skip-accessed diagnostic appears applied; reverse it before applying this overlay"
	fi

	log "applying face-GFN private TDP level diagnostic overlay"
	as_tree_owner patch -d "$SRC" -p1 -i "$PATCH_FILE"
	rg -n "$MARKER|$MMU_MARKER|COFUNC_TDX.*GFN|private_pfn|tdp_adjust|tdp_target" \
...
reverse_patch_file() {
	...
	if [[ $(patch_state) != applied ]]; then
		die "0009 overlay does not appear applied"
	fi

	log "reversing face-GFN private TDP level diagnostic overlay"
