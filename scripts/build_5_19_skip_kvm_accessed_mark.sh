...
PATCH_FILE=${PATCH_FILE:-$BUNDLE/patches/host-kernel/0004-Diagnostic-skip-KVM-accessed-mark-on-pfn-release.patch}
JOBS=${JOBS:-16}
STAMP=$(date -u +%Y%m%d_%H%M%S)
BACKUP_DIR=${BACKUP_DIR:-$BUNDLE/backups/host-kernel-skip-kvm-accessed-$STAMP}
...
accessed-mark diagnostic patch to the 5.19 source tree, and rebuilds
arch/x86/kvm.
...
patch_state() {
	if rg -q 'Diagnostic for old-ABI TDX bad-page reports' "$SRC/virt/kvm/kvm_main.c"; then
		echo "applied"
	else
		echo "not-applied"
	fi
...
apply_patch_file() {
	...
	log "applying KVM accessed-mark diagnostic patch"
	as_tree_owner patch -d "$SRC" -p1 -i "$PATCH_FILE"
	rg -n 'Diagnostic for old-ABI TDX bad-page reports|mark_page_accessed|kvm_set_page_accessed' \
...
reverse_patch_file() {
	...
	log "reversing KVM accessed-mark diagnostic patch"
