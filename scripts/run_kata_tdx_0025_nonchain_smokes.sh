#!/usr/bin/env bash
# One-shot true-4K non-chain smoke harness. Each workload receives its own
# Fig. 11 smoke run and kernel delta. The harness never retries a workload.
set -Eeuo pipefail
...
run_root=${RUN_ROOT:-/home/booklyn/BookArchive/StageBreakdownRuns/kata_tdx_0025_nonchain_smokes_$(date -u +%Y%m%d_%H%M%S)}
batch_token=${RUN_TOKEN:-n25-$(date -u +%H%M%S)-$((BASHPID % 100))}
workloads=(
	fn_py_compression
...
finish() {
	...
	capture_cri after
	sudo -n "$GATE" post-0025-nonchain-smokes 2>&1 | tee "$run_root/postflight.log"
...
} >"$run_root/nonchain-smokes-command.txt"
cp "$0" "$run_root/harness.sh"
sha256sum "$0" >"$run_root/harness.sha256"

sudo -n "$GATE" pre-0025-nonchain-smokes-launch | tee "$run_root/preflight.log"
...
curl --connect-timeout 5 --max-time 15 -fsS http://127.0.0.1:8080/ >/dev/null
