#!/usr/bin/env bash
# One-shot Fig. 11 video measurement pilot under the loaded patch-0023/0025
# modules.  It performs one untimed cold warm-up then exactly five measured
# cold launches.  Each VM launch receives its own dmesg delta and reject check.
set -Eeuo pipefail
...
WORKLOAD=fn_py_video_processing
EXPECTED_SAMPLES=5
run_root=${RUN_ROOT:-/home/booklyn/BookArchive/StageBreakdownRuns/kata_tdx_0025_video_measurement_pilot_$(date -u +%Y%m%d_%H%M%S)}
batch_token=${RUN_TOKEN:-v25-$(date -u +%H%M%S)-$((BASHPID % 100))}
...
verify_sample_set() {
	...
	jq -e -s '
		length == 5 and all(.[]; type == "object" and
...
finish() {
	...
	capture_cri after
	sudo -n "$GATE" post-0025-video-measurement-pilot 2>&1 | tee "$run_root/postflight.log"
...
} >"$run_root/video-measurement-command.txt"
cp "$0" "$run_root/harness.sh"
sha256sum "$0" >"$run_root/harness.sha256"

sudo -n "$GATE" pre-0025-video-measurement-pilot-launch | tee "$run_root/preflight.log"
{
