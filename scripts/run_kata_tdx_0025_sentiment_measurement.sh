#!/usr/bin/env bash
# One-shot Fig. 11 sentiment measurement collection under the loaded
...
WORKLOAD=fn_py_sentiment
EXPECTED_SAMPLES=20
run_root=${RUN_ROOT:-/home/booklyn/BookArchive/StageBreakdownRuns/kata_tdx_0025_sentiment_measurement_$(date -u +%Y%m%d_%H%M%S)}
batch_token=${RUN_TOKEN:-s25-$(date -u +%H%M%S)-$((BASHPID % 100))}
...
finish() {
	...
	capture_cri after
	sudo -n "$GATE" post-0025-sentiment-measurement 2>&1 | tee "$run_root/postflight.log"
...
} >"$run_root/sentiment-measurement-command.txt"
cp "$0" "$run_root/harness.sh"
sha256sum "$0" >"$run_root/harness.sha256"

sudo -n "$GATE" pre-0025-sentiment-measurement-launch | tee "$run_root/preflight.log"
{
