...
PATCH_FILE=${PATCH_FILE:-$BUNDLE/patches/host-kernel/0011-Diagnostic-log-face-gfn-hugepage-decision.patch}
JOBS=${JOBS:-16}
STAMP=$(date -u +%Y%m%d_%H%M%S)
BACKUP_DIR=${BACKUP_DIR:-$BUNDLE/backups/host-kernel-log-face-gfn-hugepage-decision-$STAMP}
MARKER='CoFunc old-ABI face-GFN hugepage decision diagnostic'
BASE_0010_MARKER='CoFunc old-ABI face-GFN all TDP level diagnostic'
...
--apply-build remounts /mnt/new_disk read-write if needed, applies the 0011
hugepage-decision diagnostic on top of the already-applied 0010 all-TDP
face-GFN overlay, and rebuilds arch/x86/kvm.

--reverse-build reverses only the 0011 diagnostic, returning the source to the
0010 all-TDP diagnostic state. It does not install or reload modules; use
...
base_0010_state() {
	if rg -q "$BASE_0010_MARKER" "$SRC/arch/x86/kvm/mmu/tdp_mmu.c"; then
		echo "applied"
	else
		echo "not-applied"
	fi
...
base_0009_mmu_state() {
	if rg -q "$BASE_0009_MMU_MARKER" "$SRC/arch/x86/kvm/mmu/mmu.c"; then
		echo "applied"
	else
		echo "not-applied"
	fi
...
check_state() {
	...
	echo "0011 face-GFN hugepage-decision state: $(patch_state)"
	echo "0010 all-face-GFN TDP base state: $(base_0010_state)"
	echo "0009 face-GFN private MMU base state: $(base_0009_mmu_state)"
...
apply_patch_file() {
	...
	if [[ $(patch_state) == applied ]]; then
		die "0011 diagnostic already appears applied"
	fi
	if [[ $(base_0010_state) != applied ]]; then
		die "0010 all-face-GFN TDP diagnostic must be applied before this diagnostic"
	fi
	if [[ $(base_0009_mmu_state) != applied ]]; then
		die "0009 face-GFN private MMU diagnostic base must be applied before this diagnostic"
	fi
	...
	log "applying face-GFN hugepage decision diagnostic"
	as_tree_owner patch -d "$SRC" -p1 -i "$PATCH_FILE"
	rg -n "$MARKER|hp_max_mapping|hp_adjust" \
...
reverse_patch_file() {
	...
	if [[ $(patch_state) != applied ]]; then
		die "0011 diagnostic does not appear applied"
	fi

	log "reversing face-GFN hugepage decision diagnostic"
