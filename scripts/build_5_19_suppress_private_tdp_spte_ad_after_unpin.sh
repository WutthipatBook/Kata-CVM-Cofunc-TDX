...
PATCH_FILE=${PATCH_FILE:-$BUNDLE/patches/host-kernel/0016-Diagnostic-suppress-private-tdp-spte-ad-after-unpin.patch}
JOBS=${JOBS:-16}
STAMP=$(date -u +%Y%m%d_%H%M%S)
BACKUP_DIR=${BACKUP_DIR:-$BUNDLE/backups/host-kernel-suppress-private-tdp-spte-ad-after-unpin-$STAMP}
MARKER='CoFunc old-ABI private TDP SPTE A/D suppress'
OLDABI_RELEASE_MARKER='CoFunc old-ABI private fault release page-flag cleanup diagnostic'
...
--apply-build remounts /mnt/new_disk read-write if needed, applies the
private-TDP-SPTE A/D suppression diagnostic to the 5.19 source tree, and
rebuilds arch/x86/kvm.
...
check_state() {
	...
	echo "0016 private-TDP-SPTE A/D suppression state: $(patch_state)"
...
apply_patch_file() {
	...
	sha256sum "$SRC/arch/x86/kvm/mmu/mmu.c" \
		"$SRC/arch/x86/kvm/mmu/tdp_mmu.c" \
	...
	if [[ $(patch_state) == applied ]]; then
		die "0016 diagnostic already appears applied"
	fi
	if [[ $(oldabi_release_state) != applied ]]; then
		die "0015 private-fault release diagnostic must be applied before 0016"
	fi
	if [[ $(tdx_unpin_state) != applied ]]; then
		die "0014 TDX unpin diagnostic must be applied before 0016"
	fi
	if [[ $(oldabi_2m_state) != applied ]]; then
		die "0013 old-ABI private 2M promotion must be applied before 0016"
	fi

	log "applying private-TDP-SPTE A/D suppression diagnostic"
	as_tree_owner patch -d "$SRC" -p1 -i "$PATCH_FILE"
	rg -n "$MARKER|private fault release range|TDX unpin range|cofunc_tdx_private_spte_skip_ad_track|cofunc_tdx_clear_page_ad_flags|cofunc_tdx_oldabi_clear_page_ad_flags" \
...
reverse_patch_file() {
	...
	if [[ $(patch_state) != applied ]]; then
		die "0016 diagnostic does not appear applied"
	fi

	log "reversing private-TDP-SPTE A/D suppression diagnostic"
