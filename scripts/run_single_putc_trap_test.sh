...
main() {
	...
	log "boot output: $OUT"
	log "building single-putc trap diagnostic ISO"
	COFUNC_SINGLE_PUTC_TRAP=1 "$BUNDLE/scripts/apply_current_early_probe_build.sh" 2>&1 \
	...
	if [[ -f "$OUT/current-trace/current-serial.log" ]]; then
		...
		for pat in "[GRUB probe] before multiboot2" "[GRUB probe] gfxpayload=" \
			...
			"[GRUB probe] before boot" "[GRUB probe] boot returned" "#UD - Invalid Opcode" \
			"RIP  - " "R10 -" "R11  -" "[ASM0]" "[ASM1]" "[ChCore probe]" "[ChCore]"; do
			printf '%s=' "$pat"
			LC_ALL=C grep -aF "$pat" "$OUT/current-trace/current-serial.log" | wc -l
		done
	fi

	if [[ -f "$OUT/current-cvm.log" ]]; then
		log "qemu warning summary"
		LC_ALL=C rg -n 'unknown split-container|unknown tdg\.vp\.vmcall|warning:|invalid|INVALID' "$OUT/current-cvm.log" | head -n 80 || true
	fi
