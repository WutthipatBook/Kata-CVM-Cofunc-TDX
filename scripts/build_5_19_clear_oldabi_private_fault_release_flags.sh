...
PATCH_FILE=${PATCH_FILE:-$BUNDLE/patches/host-kernel/0015-Diagnostic-clear-oldabi-private-fault-release-flags.patch}
JOBS=${JOBS:-16}
STAMP=$(date -u +%Y%m%d_%H%M%S)
BACKUP_DIR=${BACKUP_DIR:-$BUNDLE/backups/host-kernel-clear-oldabi-private-fault-release-flags-$STAMP}
MARKER='CoFunc old-ABI private fault release page-flag cleanup diagnostic'
...
old-ABI private-fault PFN release flag cleanup diagnostic to the 5.19 source
tree, and rebuilds arch/x86/kvm.
...
check_state() {
	...
	echo "0015 private-fault release flag-cleanup state: $(patch_state)"
...
apply_patch_file() {
	...
	if [[ $(patch_state) == applied ]]; then
		die "0015 diagnostic already appears applied"
	fi
	if [[ $(oldabi_2m_state) != applied ]]; then
		die "0013 old-ABI private 2M promotion must be applied before this diagnostic"
	fi

	log "applying old-ABI private-fault release flag cleanup diagnostic"
	as_tree_owner patch -d "$SRC" -p1 -i "$PATCH_FILE"
	rg -n "$MARKER|cofunc_tdx_oldabi_release_private_fault_pfn_clean|ClearPageReferenced|ClearPageDirty" \
...
reverse_patch_file() {
	...
	if [[ $(patch_state) != applied ]]; then
		die "0015 diagnostic does not appear applied"
	fi

	log "reversing old-ABI private-fault release flag cleanup diagnostic"
