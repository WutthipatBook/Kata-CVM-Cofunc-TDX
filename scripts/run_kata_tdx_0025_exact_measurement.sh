#!/usr/bin/env bash
# Generic artifact-exact Fig. 11 measurement under the loaded patch-0023/0025
# modules. It records exactly N fresh cold launches with no discarded warm-up.
# Each launch gets its own dmesg delta and cleanup check; the first unsafe
# result terminates the run without retry.
set -Eeuo pipefail
...
run_root=${RUN_ROOT:-/home/booklyn/BookArchive/StageBreakdownRuns/kata_tdx_0025_${SAFE_WORKLOAD}_exact_measurement_$(date -u +%Y%m%d_%H%M%S)}
batch_token=${RUN_TOKEN:-x25-$(date -u +%H%M%S)-$((BASHPID % 100))}
...
verify_sample_set() {
	local root="$run_root/log/$WORKLOAD"
...
finish() {
	...
	capture_cri after
	sudo -n "$GATE" "post-0025-${SAFE_WORKLOAD}-exact-measurement" 2>&1 |
		tee "$run_root/postflight.log"
...
(( $# == 2 )) || fail "usage: $0 <workload-path> <sample-count>"
[[ -n $WORKLOAD && $WORKLOAD != /* && $WORKLOAD != *"//"* ]] ||
	fail "invalid workload path: $WORKLOAD"
[[ "/$WORKLOAD/" != *"/../"* && "/$WORKLOAD/" != *"/./"* ]] ||
	fail "invalid workload path: $WORKLOAD"
[[ $EXPECTED_SAMPLES =~ ^[1-9][0-9]*$ ]] || fail "sample count must be positive"
...
} >"$run_root/exact-measurement-command.txt"
cp "$0" "$run_root/harness.sh"
sha256sum "$0" >"$run_root/harness.sha256"

sudo -n "$GATE" "pre-0025-${SAFE_WORKLOAD}-exact-measurement-launch" |
	tee "$run_root/preflight.log"
...
# Preparation validates workload-specific helpers, stages input,
# builds/imports the image, and pins the CRI image. It does not create a CRI
# pod and is outside all timings.
run_prebuild
