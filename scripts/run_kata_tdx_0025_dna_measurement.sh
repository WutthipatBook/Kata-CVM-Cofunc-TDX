#!/usr/bin/env bash
# One-shot Fig. 11 DNA measurement collection under the loaded patch-0023/0025
# modules. It performs one untimed cold warm-up then exactly ten measured cold
# launches. Each VM launch receives its own dmesg delta and reject check.
set -Eeuo pipefail
...
EXPECTED_SAMPLES=10
run_root=${RUN_ROOT:-/home/booklyn/BookArchive/StageBreakdownRuns/kata_tdx_0025_dna_measurement_$(date -u +%Y%m%d_%H%M%S)}
batch_token=${RUN_TOKEN:-d25-$(date -u +%H%M%S)-$((BASHPID % 100))}
...
finish() {
	...
	capture_cri after
	sudo -n "$GATE" post-0025-dna-measurement 2>&1 | tee "$run_root/postflight.log"
...
} >"$run_root/dna-measurement-command.txt"
cp "$0" "$run_root/harness.sh"
sha256sum "$0" >"$run_root/harness.sha256"

sudo -n "$GATE" pre-0025-dna-measurement-launch | tee "$run_root/preflight.log"
{
