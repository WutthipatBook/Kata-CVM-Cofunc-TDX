...
PATCH_FILE=${PATCH_FILE:-$BUNDLE/patches/host-kernel/0010-Diagnostic-log-all-face-gfn-tdp-mappings.patch}
JOBS=${JOBS:-16}
STAMP=$(date -u +%Y%m%d_%H%M%S)
BACKUP_DIR=${BACKUP_DIR:-$BUNDLE/backups/host-kernel-log-all-face-gfn-tdp-levels-$STAMP}
MARKER='CoFunc old-ABI face-GFN all TDP level diagnostic'
BASE_0009_MARKER='CoFunc old-ABI face-GFN private TDP level diagnostic'
BASE_0009_MMU_MARKER='CoFunc old-ABI face-GFN private MMU level diagnostic'
...
--apply-build remounts /mnt/new_disk read-write if needed, applies the 0010
all-TDP face-GFN overlay on top of the already-applied 0009 face-GFN private
diagnostic, and rebuilds arch/x86/kvm.

--reverse-build reverses only the 0010 overlay, returning the source to the
0009 diagnostic state. It does not install or reload modules; use
...
base_0009_state() {
	if rg -q "$BASE_0009_MARKER|$BASE_0009_MMU_MARKER" "$SRC/arch/x86/kvm/mmu/mmu.c" "$SRC/arch/x86/kvm/mmu/tdp_mmu.c"; then
		echo "applied"
	else
		echo "not-applied"
	fi
...
check_state() {
	...
	echo "0010 all-face-GFN TDP overlay state: $(patch_state)"
	echo "0009 face-GFN private base state: $(base_0009_state)"
...
apply_patch_file() {
	...
	if [[ $(patch_state) == applied ]]; then
		die "0010 overlay already appears applied"
	fi
	if [[ $(base_0009_state) != applied ]]; then
		die "0009 face-GFN private diagnostic must be applied before this overlay"
	fi
	...
	log "applying all-face-GFN TDP level diagnostic overlay"
	as_tree_owner patch -d "$SRC" -p1 -i "$PATCH_FILE"
	rg -n "$MARKER|COFUNC_TDX_TDP.*GFN|tdp_adjust|tdp_target" \
...
reverse_patch_file() {
	...
	if [[ $(patch_state) != applied ]]; then
		die "0010 overlay does not appear applied"
	fi

	log "reversing all-face-GFN TDP level diagnostic overlay"
