...
main() {
	...
	log "boot output: $OUT"
	log "building after-ASM0 trap diagnostic ISO"
	COFUNC_AFTER_ASM0_TRAP=1 "$BUNDLE/scripts/apply_current_early_probe_build.sh" 2>&1 \
	...
	if [[ -f "$OUT/current-trace/current-serial.log" ]]; then
		...
		for pat in "[GRUB probe] before multiboot2" "[GRUB probe] gfxpayload=" \
			...
			"RIP  - " "[ASM0]" "[ASM1]" "[ChCore probe]" "[ChCore]"; do
			printf '%s=' "$pat"
