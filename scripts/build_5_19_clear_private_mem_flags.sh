...
PATCH_FILE=${PATCH_FILE:-$BUNDLE/patches/host-kernel/0006-Diagnostic-clear-private-memfile-page-flags.patch}
JOBS=${JOBS:-16}
STAMP=$(date -u +%Y%m%d_%H%M%S)
BACKUP_DIR=${BACKUP_DIR:-$BUNDLE/backups/host-kernel-clear-private-mem-flags-$STAMP}
MARKER='CoFunc old-ABI private PFN flag cleanup diagnostic'
...
memfile page-flag diagnostic patch to the 5.19 source tree, and rebuilds
arch/x86/kvm.
...
as_tree_owner() {
	local owner
	owner=$(stat -c %U "$SRC/include/linux/kvm_host.h")
...
patch_state() {
	if rg -q "$MARKER" "$SRC/include/linux/kvm_host.h" "$SRC/arch/x86/kvm/vmx/tdx.c"; then
		echo "applied"
	else
		echo "not-applied"
	fi
...
apply_patch_file() {
	...
	ensure_rw
	mkdir -p "$BACKUP_DIR"
	cp -a "$SRC/include/linux/kvm_host.h" "$BACKUP_DIR/kvm_host.h.before"
	cp -a "$SRC/arch/x86/kvm/vmx/tdx.c" "$BACKUP_DIR/tdx.c.before"
	sha256sum "$SRC/include/linux/kvm_host.h" \
	...
	log "applying private memfile page-flag diagnostic patch"
	as_tree_owner patch -d "$SRC" -p1 -i "$PATCH_FILE"
	rg -n "$MARKER|kvm_private_mem_clear_page_flags|private_slot" \
		"$SRC/include/linux/kvm_host.h" \
...
reverse_patch_file() {
	...
	ensure_rw
	mkdir -p "$BACKUP_DIR"
	cp -a "$SRC/include/linux/kvm_host.h" "$BACKUP_DIR/kvm_host.h.before-reverse"
	...
	log "reversing private memfile page-flag diagnostic patch"
